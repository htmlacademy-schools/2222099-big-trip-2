...
import { defaultPointDate } from '../utils/event-date.js';
...
const renderDatesTrip = (points) => {
  ...
  const startDate = points[0].dateFrom ? defaultPointDate(points[0].dateFrom) : '';
  const endDate = points[points.length - 1].dateTo ? defaultPointDate(points[points.length - 1].dateTo) : '';
  return `${startDate}&nbsp;&mdash;&nbsp;${endDate}`;
};

const createTripInfo = (points, destinations, offers) => {
...
export default class ViewAdventureInfo extends AbstractView {
  #points = null;
  #destinations = null;
  #offers = null;
