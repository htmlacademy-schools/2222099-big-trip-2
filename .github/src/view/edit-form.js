...
import { dateTimeGet } from '../utils/event-date.js';
import { PointType, PointTypeDescription } from '../utils/consts.js';
...
const renderEditPointDateTemplate = (dateFrom, dateTo, isDisabled) => (
  `<div class="event__field-group  event__field-group--time">
    ...
    <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${dateTimeGet(dateFrom)}"
    ${isDisabled ? 'disabled' : ''}>
    &mdash;
    <label class="visually-hidden" for="event-end-time-1">To</label>
    <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${dateTimeGet(dateTo)}"
    ${isDisabled ? 'disabled' : ''}>
  </div>`
);
...
export default class ViewEditForm extends AbstractStatefulView {
  ...
  #submitHandler = (evt) =>{
    evt.preventDefault();
    this._callback.submit(ViewEditForm.parseStateToPoint(this._state));
  };

  #formDeleteClickHandler = (evt) => {
    evt.preventDefault();
    this._callback.deleteClick(ViewEditForm.parseStateToPoint(this._state));
  };
