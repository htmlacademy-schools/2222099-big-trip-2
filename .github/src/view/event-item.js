...
import {defaultPointDate, continuance, getDate, getTime } from '../utils/event-date.js';
import he from 'he';
...
const createWayPoint = (point, destinations, allOffers) => {
  ...
  const pointDuration = continuance(dateFrom, dateTo);
  const startDate = dateFrom !== null ? defaultPointDate(dateFrom) : '';
  const endDate = dateTo !== null ? defaultPointDate(dateTo) : '';
...
export default class ViewWayPoint extends AbstractView {
  ...
  get template(){
    return createWayPoint(this.#point, this.#destination, this.#offers);
  }
